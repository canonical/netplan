...
write_ovs_systemd_unit(const char* id, const GString* cmds, const char* generator_dir, gboolean physical, gboolean cleanup, const char* dependency, GError** error)
{
    ...
    g_autofree char* path = g_strjoin(NULL, generator_dir, "/netplan-ovs-", escaped_netdef_id, ".service", NULL);
 ...
 */
gboolean
_netplan_netdef_write_sd_ovs(const NetplanState* np_state, const NetplanNetDefinition* def, const char* generator_dir, gboolean* has_been_written, GError** error)
{
    g_assert(generator_dir != NULL);
    ...
    if (def->backend == NETPLAN_BACKEND_OVS) {
        ...
        // FIXME: This cannot be done inside a systemd generator.
        /*
 ...
 */
gboolean
_netplan_state_finish_sd_ovs_write(const NetplanState* np_state, const char* generator_dir, GError** error)
{
    g_assert(generator_dir != NULL);
    const NetplanOVSSettings* settings = &np_state->ovs_settings;
