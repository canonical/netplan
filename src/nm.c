 ...
 */
STATIC void
write_fallback_key_value(gpointer key_id, gpointer value, gpointer user_data)
{
    GKeyFile *kf = user_data;
    GHashTableIter iter;
    GHashTable* group = value;
    gpointer k, v;
     ...
     */
    if (g_hash_table_size(group) == 0) {
        g_key_file_set_string(kf, key_id, NETPLAN_NM_EMPTY_GROUP, "");
        g_key_file_remove_key(kf, key_id, NETPLAN_NM_EMPTY_GROUP, NULL);
        return;
    }
    ...
    while (g_hash_table_iter_next(&iter, &k, &v)) {
        gboolean has_key = g_key_file_has_key(kf, key_id, k, NULL);
        g_autofree gchar* old_key = g_key_file_get_string(kf, key_id, k, NULL);
        g_key_file_set_string(kf, key_id, k, v);
        ...
        if (g_strcmp0(k, "ip6-privacy") == 0 && g_strcmp0(v, "-1") == 0) {
            g_debug("NetworkManager: default override: clearing %s.%s", (gchar*)key_id, (gchar*)k);
            g_key_file_remove_key(kf, key_id, k, NULL);
        } else if (!has_key) {
            g_debug("NetworkManager: passing through fallback key: %s.%s=%s", (gchar*)key_id, (gchar*)k, (gchar*)v);
            g_key_file_set_comment(kf, key_id, k, "Netplan: passthrough setting", NULL);
        } else if (g_strcmp0(v, old_key) != 0) {
            g_debug("NetworkManager: fallback override: %s.%s=%s", (gchar*)key_id, (gchar*)k, (gchar*)v);
            g_key_file_set_comment(kf, key_id, k, "Netplan: passthrough override", NULL);
        }
    }
}
