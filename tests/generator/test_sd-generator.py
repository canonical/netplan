...
''')
        ...
        self.assertEqual(
            os.listdir(os.path.join(self.workdir.name, 'run', 'systemd', 'generator',
        ...
        self.assertEqual(
            sorted(os.listdir(os.path.join(self.workdir.name, 'run', 'systemd',
                                           'generator.late', 'systemd-networkd-wait-online.service.d'))),
            ['10-netplan.conf'])
        # systemd-networkd.service.wants
        self.assertEqual(
            sorted(os.listdir(os.path.join(self.workdir.name, 'run', 'systemd',
                                           'generator.late', 'systemd-networkd.service.wants'))),
            ['netplan-ovs-cleanup.service', 'netplan-wpa-wl0.service'])
        # network.target.wants
        self.assertEqual(
            sorted(os.listdir(os.path.join(self.workdir.name, 'run', 'systemd',
                                           'generator.late', 'network.target.wants'))),
            ['netplan-regdom.service'])
        # multi-user.target.wants
        self.assertEqual(
            sorted(os.listdir(os.path.join(self.workdir.name, 'run', 'systemd',
                                           'generator.late', 'multi-user.target.wants'))),
